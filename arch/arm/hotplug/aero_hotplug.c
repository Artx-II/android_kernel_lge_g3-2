...
#define DEFAULT_UP_FREQUENCY	1497600
...
static void aero_hotplug_suspend(struct early_suspend *handler)
{
	queue_work(system_power_efficient_wq, &suspend);
}


static void aero_hotplug_resume(struct early_suspend *handler)
{
	queue_work(system_power_efficient_wq, &resume);
}

static struct early_suspend aero_hotplug_power_suspend = {
...
#ifdef CONFIG_EARLYSUSPEND
	register_early_suspend(&aero_hotplug_power_suspend);
